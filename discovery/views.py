...
def analyze_opportunity(request, pk):
    ...
    source_type = AnalysisRun.SOURCE_EBAY_API if discovered.source == 'ebay_api' else AnalysisRun.SOURCE_CSV_FALLBACK
    analysis_run = AnalysisRun.objects.create(
        title=f"Oportunitate analizată - {discovered.title}"[:255],
        source_type=source_type,
        keyword=discovered.search_query.keyword,
        products_count=1,
        status=AnalysisRun.STATUS_COMPLETED,
        notes=f"Produs selectat din căutarea: {discovered.search_query.keyword}. Sursă folosită: {discovered.source}.",
    )
